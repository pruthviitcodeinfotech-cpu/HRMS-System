...
class PayrollGroupRepository(BaseRepository[PayrollGroup]):
    ...
    async def search(self, org_id: int, *, page: int = 1, page_size: int = 25) -> list[PayrollGroup]:
...
class PayrollColumnSettingRepository(BaseRepository[PayrollColumnSetting]):
    ...
    async def replace_columns(
        ...
        delete_stmt = delete(PayrollColumnSetting).where(PayrollColumnSetting.payroll_group_id == group_id)
    ...
    async def search(
        ...
        *,
        ...
        if payment_status is not None:
            val = payment_status.value if isinstance(payment_status, PaymentStatus) else payment_status
        ...
        stmt = select(FinalizedPayrollRun).where(and_(*conds)).order_by(desc(FinalizedPayrollRun.cycle_from))
        stmt = apply_pagination(stmt, page, page_size)
        return list((await self.session.execute(stmt)).scalars().all())
...
class PayrollComputedRowRepository(BaseRepository[PayrollComputedRow]):
    ...
    async def get_summary(
        ...
        stmt = select(
            ...
            func.coalesce(func.sum(PayrollComputedRow.gross_earnings), 0).label("total_gross_earnings"),
            ...
            func.coalesce(func.sum(PayrollComputedRow.penalties_amount), 0).label("total_penalties"),
            func.coalesce(func.sum(PayrollComputedRow.loan_advance_deduction), 0).label("total_deductions"),
        ).join(
            PayrollGroup, PayrollGroup.id == PayrollComputedRow.payroll_group_id
        ).where(
            PayrollGroup.org_id == org_id,
    ...
    async def search(
        ...
        *,
        ...
        if adjustment_source is not None:
            val = adjustment_source.value if isinstance(adjustment_source, AdjustmentSource) else adjustment_source
