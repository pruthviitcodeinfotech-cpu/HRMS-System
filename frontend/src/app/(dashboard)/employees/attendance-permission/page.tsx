...
import { useDevicesList } from "@/features/dashboard/hooks";
...
export default function AttendancePermissionPage() {
  ...
  const [localOverrides, setLocalOverrides] = useState<
    Record<number, Partial<AttendancePermissionItem>>
  >({});
  ...
  // Map real employee items from database
  const items = useMemo<AttendancePermissionItem[]>(() => {
    const rawList = employeesQuery.data?.items || [];
    return rawList.map((emp) => {
      const override = localOverrides[emp.employee_id] || {};
      return {
        id: emp.employee_id,
        employee_id: emp.employee_code || String(emp.employee_id),
        name: emp.employee_name || emp.display_name || `Employee #${emp.employee_id}`,
        department: emp.department_name || "-",
        designation: emp.designation_name || "-",
        biometrics: "Fingerprint",
        method: override.method || "Hardware Device",
        mobile_attendance: override.mobile_attendance ?? false,
        geofencing: override.geofencing || "-",
        auto_punch_out: override.auto_punch_out || "-",
      };
    });
  }, [employeesQuery.data, localOverrides]);
  ...
  const handleToggleMobile = (id: number) => {
    setLocalOverrides((prev) => ({
      ...
      [id]: {
        ...prev[id],
        mobile_attendance: !(items.find((i) => i.id === id)?.mobile_attendance ?? false),
      },
    }));
  ...
  const handleBulkAction = (field: "mobile" | "geofencing" | "auto_punch_out", value: boolean) => {
    ...
    setLocalOverrides((prev) => {
      ...
      selectedIds.forEach((id) => {
        const cur = next[id] || {};
        if (field === "mobile") {
          cur.mobile_attendance = value;
        } else if (field === "geofencing") {
          cur.geofencing = value ? "Enabled" : "Disabled";
        } else {
          cur.auto_punch_out = value ? "Enabled" : "Disabled";
        }
        next[id] = cur;
      });
      return next;
      ...
      <div className="p-6 space-y-6 bg-slate-50/40 min-h-screen">
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 p-4 rounded-xl shadow-xs flex flex-wrap items-center gap-6">
          ...
          <div className="flex flex-wrap items-center gap-4 flex-1">
            {/* Branch Selector — Master Data single source of truth */}
              ...
              <div className="relative">
                <select
                  ...
                  onChange={(e) => {
                    ...
                    if (val) setSelectedBranchId(val);
                  }}
            ...
            {/* Device Selector — Dynamic from hardware devices */}
            <div className="flex flex-col gap-1">
              <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">
                Choose Device
              </span>
        ...
        <div className="w-full overflow-x-auto rounded-xl border border-slate-200/80 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-xs">
          {employeesQuery.isLoading ? (
            <div className="p-12 text-center flex flex-col items-center justify-center gap-3">
              ...
              <p className="text-sm font-medium text-slate-500">Loading branch employees...</p>
            </div>
            ...
            <div className="p-12 text-center">
              ...
              <p className="text-xs text-slate-400 mt-1">
                Select a different branch or adjust search filters.
              </p>
            </div>
          ) : (
